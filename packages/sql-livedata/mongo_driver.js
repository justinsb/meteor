/**
 * Provide a synchronous Collection API using fibers, backed by Sql. This is
 * only for use on the server, and mostly identical to the client API.
 * 
 * NOTE: the public API methods must be run within a fiber. If you call these
 * outside of a fiber they will explode!
 */

var path = Npm.require('path');
var MysqlNpm = Npm.require('mysql');
...
// return new Meteor.SqlCollection.ObjectID(document.toHexString());
// }
...
// }
// if (document instanceof Meteor.SqlCollection.ObjectID) {
...
SqlCollection = function (connection, collectionName) {
  var self = this;
  self.connection = connection;
  self.collectionName = collectionName;
  
  // TODO: This is not safe
  self.tableName = collectionName;
  
  var metadataFuture = new Future;
  var client = self.connection.db;
  client.getMetadata(self.tableName, function (err, results) {
    if (err) {
      metadataFuture.throw(err);
    } else {
      metadataFuture.return(results);
    }
  });
  self._metadata = metadataFuture.wait();
...
SqlCollection.prototype.find = function(selector, fields, options, callback) {
  var self = this;
  Meteor._debug("SqlCollection.find(" + JSON.stringify(arguments) + ")");
  var err = null;
  var result = new SqlCursor(self, selector, fields, options);
  if (callback) {
    callback(err, result);
  }
...
SqlCollection.prototype.insert = function (docs, options, callback) {
  var self = this;
  Meteor._debug("SqlCollection.insert(" + JSON.stringify(arguments) + ")");
  ...
  var client = self.connection.db;
  client.insertAll(self.tableName, self._metadata, docs, options, deferredCallback);
};


SqlCollection.prototype.update = function (selector, mod, options, callback) {
  Meteor._debug("SqlCollection.update(" + JSON.stringify(arguments) + ")");
  var self = this;
...
SqlCollection.prototype.remove = function (selector, options, callback) {
  Meteor._debug("SqlCollection.remove(" + JSON.stringify(arguments) + ")");
  var self = this;
...
SqlCursor = function (collection, selector, fields, options) {
  var self = this;
...
SqlCursor.prototype.nextObject = function (callback) {
  var self = this;
  Meteor._debug("SqlCursor.nextObject()");
...
// Meteor._debug("Matches: " + JSON.stringify(self._results));
  ...
  // result._id = new SQLDB.ObjectID(id);
  // }
  // }
      }
    }
...
SqlCursor.prototype.count = function (applySkipLimit, callback) {
  var self = this;
  Meteor._debug("SqlCursor.count(" + JSON.stringify(arguments) + ")");
...
SqlCursor.prototype._getResults = function (callback) {
  var self = this;
  ...
  client.getAll(self.collection._metadata, matcher, self.sorter, self.skip, self.limit, self.projection, function (err, results) {
    if (!err) {
...
SqlCursor.prototype.rewind = function () {
  // Polling driver seems to rely on the fact that rewind doesn't use a snapshot
  var self = this;
  Meteor._debug("SqlCursor.rewind(" + JSON.stringify(arguments) + ")");
  self._results = null;
  self._nextPos = 0;
};

SqlConnection = function (url, options) {
  var self = this;
  options = options || {};
  ...
  var sqlOptions = {db: {safe: true}, server: {}, replSet: {}};
  ...
  if (!(/[\?&]auto_?[rR]econnect=/.test(url))) {
    sqlOptions.server.auto_reconnect = true;
  }
  ...
  if (!(/[\?&]native_?[pP]arser=/.test(url))) {
    sqlOptions.db.native_parser = false;
  }
  ...
  if (_.has(options, 'poolSize')) {
    ...
    sqlOptions.server.poolSize = options.poolSize;
    sqlOptions.replSet.poolSize = options.poolSize;
  }
  
  var options = {};
  self.db = new SqlClient(url, options);

// MysqlNpm.connect(url, mongoOptions, Meteor.bindEnvironment(function(err, db)
// {
// if (err)
// throw err;
// self.db = db;
...
SqlConnection.prototype._withDb = function (callback) {
  var self = this;
  if (self.db) {
    callback(self.db);
  } else {
...
SqlConnection.prototype._getCollection = function (collectionName) {
  var self = this;

  var collection = self._collections[collectionName];
  if (!collection) {
    collection = new SqlCollection(self, collectionName);
...
SqlConnection.prototype._createCappedCollection = function (collectionName,
                                                              byteSize) {
  var self = this;
  var future = new Future();
  self._withDb(function (db) {
...
SqlConnection.prototype._maybeBeginWrite = function () {
  var self = this;
...
SqlConnection.prototype._onFailover = function (callback) {
...
SqlConnection.prototype._insert = function (collection_name, document,
                                              callback) {
  var self = this;
...
// fence.
SqlConnection.prototype._refresh = function (collectionName, selector) {
  var self = this;
...
SqlConnection.prototype._remove = function (collection_name, selector,
                                              callback) {
  var self = this;
...
SqlConnection.prototype._dropCollection = function (collectionName, cb) {
  var self = this;
...
SqlConnection.prototype._update = function (collection_name, selector, mod,
                                              options, callback) {
  var self = this;
...
// exposed for testing
SqlConnection._isCannotChangeIdError = function (err) {
    ...
    collection.update(selector, replacementWithId, mongoOptsForInsert,
                      bindEnvironmentForWrite(function (err, result) {
                        if (err) {
                          ...
                          if (SqlConnection._isCannotChangeIdError(err)) {
                            doUpdate();
                          } else {
                            callback(err);
                          }
...
_.each(["insert", "update", "remove", "dropCollection"], function (method) {
  SqlConnection.prototype[method] = function (/* arguments */) {
    var self = this;
    return Meteor._wrapAsync(self["_" + method]).apply(self, arguments);
  };
});

// XXX SqlConnection.upsert() does not return the id of the inserted document
// unless you set it explicitly in the selector or modifier (as a replacement
// doc).
SqlConnection.prototype.upsert = function (collectionName, selector, mod,
                                             options, callback) {
  var self = this;
...
SqlConnection.prototype.find = function (collectionName, selector, options) {
  var self = this;
...
SqlConnection.prototype.findOne = function (collection_name, selector,
                                              options) {
  var self = this;
  if (arguments.length === 1)
    selector = {};
...
SqlConnection.prototype._ensureIndex = function (collectionName, index,
                                                   options) {
  var self = this;
...
SqlConnection.prototype._dropIndex = function (collectionName, index) {
  var self = this;
...
// documented Meteor.SqlCollection cursor API. It wraps a CursorDescription and
// a
...
CursorDescription = function (collectionName, selector, options) {
  var self = this;
  self.collectionName = collectionName;
  self.selector = Meteor.SqlCollection._rewriteSelector(selector);
  self.options = options || {};
};

Cursor = function (db, cursorDescription) {
  var self = this;
...
_.each(['forEach', 'map', 'rewind', 'fetch', 'count'], function (method) {
  Cursor.prototype[method] = function () {
    ...
    if (!self._synchronousCursor) {
      self._synchronousCursor = self._db._createSynchronousCursor(
...
Cursor.prototype._publishCursor = function (sub) {
  ...
  return Meteor.SqlCollection._publishCursor(self, sub, collection);
};
...
Cursor.prototype.observeChanges = function (callbacks) {
  ...
  return self._db._observeChanges(
    self._cursorDescription, ordered, callbacks);
};

SqlConnection.prototype._createSynchronousCursor = function(
  ...
  if (cursorOptions.tailable) {
    ...
    if (cursorDescription.collectionName === OPLOG_COLLECTION &&
      ...
      sqlOptions.oplogReplay = true;
    }
  }

  var dbCursor = collection.find(
    replaceTypes(cursorDescription.selector, replaceMeteorAtomWithMongo),
    cursorOptions.fields, sqlOptions);
...
SqlConnection.prototype.tail = function (cursorDescription, docCallback) {
  var self = this;
...
SqlConnection.prototype._observeChanges = function (
...
// new SqlInternals.MongoTimestamp(0, 0) (which is initialized to the
...
SqlConnection.prototype._observeChangesTailable = function (
    cursorDescription, ordered, callbacks) {
  var self = this;
...
// SqlInternals.MongoTimestamp = MongoDB.Timestamp;

SqlInternals.Connection = SqlConnection;
SqlInternals.NpmModule = MysqlNpm;
