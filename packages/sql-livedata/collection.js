...
Meteor.SqlCollection = function (name, options) {
  var self = this;
  if (! (self instanceof Meteor.SqlCollection))
    throw new Error('use "new" to construct a Meteor.SqlCollection');
  ...
  if (name !== null && typeof name !== "string") {
    throw new Error(
      "First argument to new Meteor.SqlCollection must be a string or null");
  }
  ...
  case 'MONGO':
    self._makeNewID = function () {
      ...
      return new Meteor.SqlCollection.ObjectID(src.hexString(24));
    };
    break;
  case 'STRING':
  default:
  ...
  if (!options._driver) {
    if (name && self._connection === Meteor.server &&
        typeof SqlInternals !== "undefined" &&
        SqlInternals.defaultRemoteCollectionDriver) {
      options._driver = SqlInternals.defaultRemoteCollectionDriver();
    } else {
...
_.extend(Meteor.SqlCollection.prototype, {
...
Meteor.SqlCollection._publishCursor = function (cursor, sub, collection) {
...
// replaced by $regex.
Meteor.SqlCollection._rewriteSelector = function (selector) {
  ...
  _.each(selector, function (value, key) {
    ...
    if (value instanceof RegExp) {
      ...
      ret[key] = _.map(value, function (v) {
        return Meteor.SqlCollection._rewriteSelector(v);
      });
    } else {
      ret[key] = value;
    }
...
_.each(["insert", "update", "remove"], function (name) {
  Meteor.SqlCollection.prototype[name] = function (/* arguments */) {
    var self = this;
    ...
    if (name === "insert") {
      ...
      if ('_id' in args[0]) {
        ...
        if (!insertId || !(typeof insertId === 'string'
              || insertId instanceof Meteor.SqlCollection.ObjectID))
      ...
      args[0] = Meteor.SqlCollection._rewriteSelector(args[0]);
        ...
        if (options && typeof options !== "function" && options.upsert) {
          ...
          if (options.insertedId) {
            if (!(typeof options.insertedId === 'string'
                  || options.insertedId instanceof Meteor.SqlCollection.ObjectID))
...
Meteor.SqlCollection.prototype.upsert = function (selector, modifier,
                                               options, callback) {
  var self = this;
...
Meteor.SqlCollection.prototype._ensureIndex = function (index, options) {
  var self = this;
  if (!self._collection._ensureIndex)
    throw new Error("Can only call _ensureIndex on server collections");
  self._collection._ensureIndex(index, options);
};
Meteor.SqlCollection.prototype._dropIndex = function (index) {
  var self = this;
  if (!self._collection._dropIndex)
    throw new Error("Can only call _dropIndex on server collections");
  self._collection._dropIndex(index);
};
Meteor.SqlCollection.prototype._dropCollection = function () {
  var self = this;
  if (!self._collection.dropCollection)
    throw new Error("Can only call _dropCollection on server collections");
  self._collection.dropCollection();
};
Meteor.SqlCollection.prototype._createCappedCollection = function (byteSize) {
  var self = this;
...
Meteor.SqlCollection.ObjectID = LocalCollection._ObjectID;
...
(function () {
  ...
  Meteor.SqlCollection.prototype.allow = function(options) {
    addValidator.call(this, 'allow', options);
  };
  Meteor.SqlCollection.prototype.deny = function(options) {
    addValidator.call(this, 'deny', options);
  };
})();


Meteor.SqlCollection.prototype._defineMutationMethods = function() {
  var self = this;
...
Meteor.SqlCollection.prototype._updateFetch = function (fields) {
  var self = this;
...
Meteor.SqlCollection.prototype._isInsecure = function () {
  var self = this;
...
Meteor.SqlCollection.prototype._validatedInsert = function (userId, doc,
                                                         generatedId) {
  var self = this;
...
Meteor.SqlCollection.prototype._validatedUpdate = function(
...
Meteor.SqlCollection.prototype._validatedRemove = function(userId, selector) {
  var self = this;
