var OplogCollection = new Meteor.SqlCollection("oplog-" + Random.id());
...
Tinytest.add("mongo-livedata - oplog - cursorSupported", function (test) {
  ...
  supported(true, "asdf");
  supported(true, 1234);
  supported(true, new Meteor.SqlCollection.ObjectID());

  supported(true, {_id: "asdf"});
  supported(true, {_id: 1234});
  supported(true, {_id: new Meteor.SqlCollection.ObjectID()});

  supported(true, {foo: "asdf",
                   bar: 1234,
                   baz: new Meteor.SqlCollection.ObjectID(),
                   eeney: true,
                   miney: false,
                   moe: null});
