...
if (Meteor.isServer) {
  Meteor.methods({
    createInsecureCollection: function (name, options) {
      ...
      var c = new Meteor.SqlCollection(name, options);
      COLLECTIONS[name] = c;
      c._insecure = true;
...
if (Meteor.isServer) {
  ...
  m[upsertTestMethod] = function (run, useUpdate, options) {
    ...
    upsertTestMethodColl = new Meteor.SqlCollection(upsertTestMethod + "_collection_" + run, options);
...
Meteor._FailureTestCollection =
  new Meteor.SqlCollection("___meteor_failure_test_collection");
...
Tinytest.addAsync("mongo-livedata - basics, " + idGeneration, function (test, onComplete) {
  ...
  var coll, coll2;
  if (Meteor.isClient) {
    coll = new Meteor.SqlCollection(null, collectionOptions) ; // local, unmanaged
    coll2 = new Meteor.SqlCollection(null, collectionOptions); // local, unmanaged
  } else {
    coll = new Meteor.SqlCollection("livedata_test_collection_"+run, collectionOptions);
    coll2 = new Meteor.SqlCollection("livedata_test_collection_2_"+run, collectionOptions);
  }
...
Tinytest.addAsync("mongo-livedata - stop handle in callback, " + idGeneration, function (test, onComplete) {
  ...
  if (Meteor.isClient) {
    ...
    coll = new Meteor.SqlCollection("stopHandleInCallback-"+run, collectionOptions);
  }
...
if (Meteor.isServer) {
  Tinytest.addAsync("mongo-livedata - recursive observe throws, " + idGeneration, function (test, onComplete) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("observeInCallback-"+run, collectionOptions);
  ...
  Tinytest.addAsync("mongo-livedata - cursor dedup, " + idGeneration, function (test, onComplete) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("cursorDedup-"+run, collectionOptions);
  ...
  Tinytest.addAsync("mongo-livedata - async server-side insert, " + idGeneration, function (test, onComplete) {
    ...
    var cname = Random.id();
    var coll = new Meteor.SqlCollection(cname);
    var doc = { foo: "bar" };
    var x = 0;
  ...
  Tinytest.addAsync("mongo-livedata - observe sorted, limited " + idGeneration, function (test, onComplete) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("observeLimit-"+run, collectionOptions);
...
testAsyncMulti('mongo-livedata - empty documents, ' + idGeneration, [
  function (test, expect) {
    ...
    var coll = new Meteor.SqlCollection(this.collectionName, collectionOptions);
...
testAsyncMulti('mongo-livedata - document with length, ' + idGeneration, [
  function (test, expect) {
    ...
    var self = this;
    var coll = self.coll = new Meteor.SqlCollection(self.collectionName, collectionOptions);
...
testAsyncMulti('mongo-livedata - document goes through a transform, ' + idGeneration, [
  function (test, expect) {
    ...
    var self = this;
    self.coll = new Meteor.SqlCollection(self.collectionName, self.collectionOptions);
    var obs;
...
testAsyncMulti('mongo-livedata - transform sets _id if not present, ' + idGeneration, [
  function (test, expect) {
    ...
    var self = this;
    self.coll = new Meteor.SqlCollection(this.collectionName, collectionOptions);
...
if (Meteor.isServer) {
  Tinytest.addAsync("mongo-livedata - update return values, " + idGeneration, function (test, onComplete) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("livedata_update_result_"+run, collectionOptions);
  ...
  Tinytest.addAsync("mongo-livedata - id-based invalidation, " + idGeneration, function (test, onComplete) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("livedata_invalidation_collection_"+run, collectionOptions);
  ...
  Tinytest.add("mongo-livedata - upsert error parse, " + idGeneration, function (test) {
    var run = test.runId();
    var coll = new Meteor.SqlCollection("livedata_upsert_errorparse_collection_"+run, collectionOptions);
  ...
  _.each([true, false], function (useUpdate) {
    _.each([true, false], function (useDirectCollection) {
      Tinytest.add("mongo-livedata - " + (useUpdate ? "update " : "") + "upsert" + (minimongo ? " minimongo" : "") + (useDirectCollection ? " direct collection " : "") + ", " + idGeneration, function (test) {
        ...
        var coll = new Meteor.SqlCollection(
        ...
        var t1 = new Meteor.SqlCollection.ObjectID();
        var t2 = new Meteor.SqlCollection.ObjectID();
...
// collections, and run the tests for both the Meteor.SqlCollection and the
// LocalCollection. On the server, we test mongo-backed collections, for both
// the Meteor.SqlCollection and the MongoConnection.
//
    ...
    _.each([true, false], function (useUpdate) {
      Tinytest.addAsync(asyncUpsertTestName(useNetwork, useDirectCollection, useUpdate, idGeneration), function (test, onComplete) {
        ...
        if (useNetwork) {
          ...
          coll = new Meteor.SqlCollection(collName, collectionOptions);
          Meteor.subscribe("c-" + collName, next0);
        } else {
          var opts = _.clone(collectionOptions);
          if (Meteor.isClient)
            opts.connection = null;
          coll = new Meteor.SqlCollection(collName, opts);
          if (useDirectCollection)
            coll = coll._collection;
        }
        ...
        var next2 = function (err, result) {
          ...
          t1 = new Meteor.SqlCollection.ObjectID();
          t2 = new Meteor.SqlCollection.ObjectID();
...
if (Meteor.isClient) {
  _.each([true, false], function (useUpdate) {
    Tinytest.addAsync("mongo-livedata - " + (useUpdate ? "update " : "") + "upsert in method, " + idGeneration, function (test, onComplete) {
      var run = test.runId();
      upsertTestMethodColl = new Meteor.SqlCollection(upsertTestMethod + "_collection_" + run, collectionOptions);
      var m = {};
...
_.each(Meteor.isServer ? [true, false] : [true], function (minimongo) {
  _.each([true, false], function (useUpdate) {
    Tinytest.add("mongo-livedata - " + (useUpdate ? "update " : "") + "upsert by id" + (minimongo ? " minimongo" : "") + ", " + idGeneration, function (test) {
      ...
      if (minimongo)
        options = _.extend({}, collectionOptions, { connection: null });
      var coll = new Meteor.SqlCollection("livedata_upsert_by_id_collection_"+run, options);
...
Tinytest.add('mongo-livedata - rewrite selector', function (test) {
  test.equal(Meteor.SqlCollection._rewriteSelector({x: /^o+B/im}),
             {x: {$regex: '^o+B', $options: 'im'}});
  test.equal(Meteor.SqlCollection._rewriteSelector({x: {$regex: /^o+B/im}}),
             {x: {$regex: '^o+B', $options: 'im'}});
  test.equal(Meteor.SqlCollection._rewriteSelector({x: /^o+B/}),
             {x: {$regex: '^o+B'}});
  test.equal(Meteor.SqlCollection._rewriteSelector({x: {$regex: /^o+B/}}),
             {x: {$regex: '^o+B'}});
  test.equal(Meteor.SqlCollection._rewriteSelector('foo'),
             {_id: 'foo'});

  test.equal(
    Meteor.SqlCollection._rewriteSelector(
      {'$or': [
        {x: /^o/},
        {y: /^p/},
        {z: 'q'},
  ...
  var oid = new Meteor.SqlCollection.ObjectID();
  test.equal(Meteor.SqlCollection._rewriteSelector(oid),
             {_id: oid});
});
...
testAsyncMulti('mongo-livedata - specified _id', [
  function (test, expect) {
    ...
    var coll = new Meteor.SqlCollection(this.collectionName);
...
_.each( ['STRING', 'MONGO'], function (idGeneration) {
  ...
  testAsyncMulti('mongo-livedata - consistent _id generation ' + name + ', ' + repetitions + ' repetitions on ' + collectionCount + ' collections, idGeneration=' + idGeneration, [ function (test, expect) {
    ...
    this.collections = _.times(collectionCount, function () {
      ...
      return (COLLECTIONS[collectionName] = new Meteor.SqlCollection(collectionName, collectionOptions));
    });
  }, function (test, expect) {
...
testAsyncMulti('mongo-livedata - empty string _id', [
  function (test, expect) {
    ...
    self.coll = new Meteor.SqlCollection(self.collectionName);
    try {
...
if (Meteor.isServer) {
  ...
  testAsyncMulti("mongo-livedata - minimongo on server to server connection", [
    function (test, expect) {
      ...
      self.id = Random.id();
      var C = self.C = new Meteor.SqlCollection("ServerMinimongo_" + self.id);
      C.allow({
    ...
    function (test, expect) {
      ...
      if (self.conn.status().connected) {
        self.miniC = new Meteor.SqlCollection("ServerMinimongo_" + self.id, {
  ...
  testAsyncMulti("mongo-livedata - minimongo observe on server", [
    function (test, expect) {
      var self = this;
      self.id = Random.id();
      self.C = new Meteor.SqlCollection("ServerMinimongoObserve_" + self.id);
      self.events = [];
    ...
    function (test, expect) {
      var self = this;
      if (self.conn.status().connected) {
        self.miniC = new Meteor.SqlCollection("ServerMinimongoObserve_" + self.id, {
          connection: self.conn
        });
        var exp = expect(function (err) {
          test.isFalse(err);
...
Tinytest.addAsync("mongo-livedata - local collections with different connections", function (test, onComplete) {
  ...
  var coll1 = new Meteor.SqlCollection(cname);
  var doc = { foo: "bar" };
  var coll2 = new Meteor.SqlCollection(cname2, { connection: null });
...
Tinytest.addAsync("mongo-livedata - local collection with null connection, w/ callback", function (test, onComplete) {
  var cname = Random.id();
  var coll1 = new Meteor.SqlCollection(cname, { connection: null });
  var doc = { foo: "bar" };
...
testAsyncMulti("mongo-livedata - update handles $push with $each correctly", [
  function (test, expect) {
    ...
    self.collection = new Meteor.SqlCollection(collectionName);
...
if (Meteor.isServer) {
  Tinytest.add("mongo-livedata - upsert handles $push with $each correctly", function (test) {
    var collection = new Meteor.SqlCollection(Random.id());
...
Meteor.isServer && Tinytest.add("mongo-livedata - oplog - _disableOplog", function (test) {
  ...
  var coll = new Meteor.SqlCollection(collName);
...
testAsyncMulti("mongo-livedata - oplog - update EJSON", [
  function (test, expect) {
    ...
    self.collection = new Meteor.SqlCollection(collectionName);
    self.date = new Date;
    self.objId = new Meteor.SqlCollection.ObjectID;
  ...
  function (test, expect) {
    ...
    self.objId2 = new Meteor.SqlCollection.ObjectID;
    runInFence(function () {
      self.collection.update(
...
Meteor.isServer && Tinytest.add("mongo-livedata - cursor dedup stop", function (test) {
  var coll = new Meteor.SqlCollection(Random.id());
...
testAsyncMulti("mongo-livedata - undefined find options", [
  ...
  function (test, expect) {
    var self = this;
    self.coll = new Meteor.SqlCollection(self.collName);
